...
# This script is the 5th out of a series of 9 scripts.
#
...
# of the targets and retrieves the identifying information of its top-5 rivals
...
# Thus, we first find the GVKEY of the targets, filter the Hoberg-Philips dataset to find
...
get_and_append_tgt_rivals_id_data <- function(working_dir, wrds_user, wrds_pass, num_of_rivals_per_firm) {
  ...
  # It also takes the number of rivals to be collected for each target from the Hoberg-Philips dataset
  ...
  # pulling the permcos
  tgt_reg_id_info <- readRDS("data/processed_data/tgt_id_data_with_permcos_and_permnos")
  ...
  # adding the firms' gvkeys to the initial dataset, i.e the one with the each target's 
  # identifying information (name, ticker, cusip, permco, etc.)
  # and record the dates that the target was added to the CRSP/COMPUSTAT linking table
  ...
  tgt_reg_id_info_with_gvkeys_aux1 <- merge(tgt_reg_id_info, tgt_id_and_gvkeys_w_permcos,
    ...
    rename(tgt_gvkey=gvkey, tgt_linkdt = linkdt, tgt_linkenddt = linkenddt) %>% group_by(merger_id, tgt_permco, tgt_permno) %>%
    distinct(tgt_gvkey, .keep_all = T)
  
  # in dealing with multiple GVKEYs retrieved for the same (permco, permno) combination,
  # we shall pick the one whose date range in CRSP is included the merger announcement date
  tgt_reg_id_info_with_gvkeys_aux2 <- tgt_reg_id_info_with_gvkeys_aux1 %>% filter( ifelse( is.na(tgt_gvkey) | n()==1 , TRUE,  
                                                                                                                          ( date_announced >= tgt_linkdt & ( date_announced <= tgt_linkenddt | is.na(tgt_linkenddt) ) 
                                                                                                                            ) 
                                                                                                             ) 
                                                                                                     ) %>%
    ungroup()
  ...
  missing_merger_ids <- setdiff(tgt_reg_id_info_with_gvkeys_aux1$merger_id, tgt_reg_id_info_with_gvkeys_aux2$merger_id)
  ...
  missing_data <- tgt_reg_id_info_with_gvkeys_aux1 %>% 
    filter(merger_id %in% missing_merger_ids) %>% group_by(merger_id, tgt_permco, tgt_permno) %>% mutate( days_diff = as.numeric(abs( date_announced - tgt_linkdt ) ) ) %>%
    slice_min(order_by = days_diff, n=1, with_ties = F) %>% select(- days_diff) %>% ungroup()
  
  # finally add them back to the dataset
  tgt_reg_id_info_with_gvkeys <- bind_rows(tgt_reg_id_info_with_gvkeys_aux2, missing_data)
  
  # once we have a unique gvkey for each firm we can access the Hoberg-Philips dataset to retrieve their competitors
  # pulling the gvkeys
  tgt_gvkeys <- tgt_reg_id_info_with_gvkeys %>% filter(!is.na(tgt_gvkey)) %>% distinct(tgt_gvkey) %>% pull()
  
  # getting all competitors for every firm in the Hoberg-Philips dataset 
  competitors_per_tgt <- competitors_txt_def %>% filter(gvkey1 %in% tgt_gvkeys) 
  
  # adding the competitors back to the original dataset
  all_tgt_and_rivals_id_aux <- merge(tgt_reg_id_info_with_gvkeys, competitors_per_tgt, by.x = "tgt_gvkey", by.y = "gvkey1", all.x = T) 
  ...
  all_tgt_and_rivals_id <- all_tgt_and_rivals_id_aux %>% group_by(merger_id, gvkey2) %>% filter( ifelse( is.na(gvkey2), TRUE, year <= year(date_announced) ) ) %>%
    filter( ifelse( is.na(gvkey2), TRUE, year == max(year) ) ) %>%
    ungroup()
  
  # since we depart from the rival's gvkey, the path to obtain their id info is the opposite of the one we took for the targets
  ...
  # all_tgt_and_rivals_id contains all the identifying information of the target plus the gvkey of all its rivals
  # the next step is to retrieve the identifying info of these competitors, which we are forced to do based 
  # on their gvkey
  tgt_rivals_gvkeys <- all_tgt_and_rivals_id %>% filter(!is.na(gvkey2)) %>% distinct( gvkey2 ) %>% pull()
  
  # so we start by getting their permanent company and issue number (permco and permno, respectively)
  tgt_rivals_gvkeys_and_permcos_aux1 <- crsp_compstat_link_table %>% select(gvkey, lpermco, lpermno, linkdt, linkenddt) %>% 
    filter(gvkey %in% tgt_rivals_gvkeys & !is.na(lpermco)) %>% distinct(gvkey, lpermco, lpermno, .keep_all = T) 
  
  # joining the rivals' permco to the dataset with the original dataset, that is, all_tgt_and_rivals_id which has the targets' id info
  # plus the rivals' gvkeys
  # so we now have all the targets' identifying information, plus the rivals' gvkeys and permcos
  # we also keep track of the dates the rivals were added to COMPUSTAT/CRSP table
  all_tgt_and_rivals_id_w_permcos_aux2 <- merge(all_tgt_and_rivals_id, tgt_rivals_gvkeys_and_permcos_aux1, by.x="gvkey2", by.y="gvkey", all.x=TRUE) %>% 
    rename(tgt_rival_gvkey=gvkey2, tgt_rival_permco=lpermco, tgt_rival_permno=lpermno, tgt_rival_linkdt=linkdt, tgt_rival_linkenddt=linkenddt)
  ...
  missing_merger_ids <- setdiff(all_tgt_and_rivals_id_aux$merger_id, all_tgt_and_rivals_id_w_permcos_aux2$merger_id)
  
  # for those merger ids we merely assign an NA to the columns imported from Hoberg-Philips dataset 
  missing_data <- all_tgt_and_rivals_id_aux %>% 
    filter(merger_id %in% missing_merger_ids) %>% group_by(merger_id, tgt_permco) %>% slice(1) %>%
    mutate( year = NA,
            gvkey2 = NA,
            score = NA
    ) %>% ungroup()
  ...
  # finally add them back to the dataset
  all_tgt_and_rivals_id_w_permcos <- bind_rows(all_tgt_and_rivals_id_w_permcos_aux2, missing_data)
  ...
  tgt_rivals_permcos <- all_tgt_and_rivals_id_w_permcos %>% filter(!is.na(tgt_rival_permco)) %>% distinct(tgt_rival_permco) %>% pull()
  tgt_rivals_permnos <- all_tgt_and_rivals_id_w_permcos %>% filter(!is.na(tgt_rival_permno)) %>% distinct(tgt_rival_permno) %>% pull()
  ...
  # we can distinguish the date record of targets and their rivals
  tgt_rivals_id <- stock_id %>% select(permco, permno, namedt, nameendt, cusip, ncusip, ticker, comnam) %>% filter(permco %in% tgt_rivals_permcos & permno %in% tgt_rivals_permnos) %>% # subset dataset on the targets' rivals
    ...
    rename(tgt_rival_end_date=nameendt, tgt_rival_start_date=namedt,
           tgt_rival_name=comnam, tgt_rival_cusip=cusip, 
           tgt_rival_ncusip=ncusip, tgt_rival_ticker=ticker) %>% 
    ungroup()
  
  # now merge it back to the table of interest: all_tgt_and_rivals_id_w_permcos which contains
  # the targets id info plus the rivals gvkey and (permco, permno)
  # this table will have all that data plus the rivals id info, i.e. name, cusip, ticker etc.
  all_tgt_and_rivals_reg_id_info_aux1 <- merge(all_tgt_and_rivals_id_w_permcos, tgt_rivals_id, by.x=c("tgt_rival_permco", "tgt_rival_permno"), 
  ...
  # the reason is that in some cases the target firm will belong to the list of target's competitors
  ...
  all_tgt_and_rivals_reg_id_info_aux2 <- all_tgt_and_rivals_reg_id_info_aux1 %>% group_by(merger_id) %>% filter( ifelse(is.na(tgt_rival_permco), TRUE, tgt_rival_start_date<= date_announced - 350 & 
                                                                                                                          (tgt_rival_end_date>= date_announced + 40 | is.na(tgt_rival_end_date) )  ) ) %>% ungroup()
  ...
  # all_tgt_and_rivals_reg_id_info_aux3 <- all_tgt_and_rivals_reg_id_info_aux2 %>% group_by(merger_id, tgt_rival_gvkey, tgt_rival_permco) %>% distinct(tgt_rival_permno, .keep_all = T) %>% 
  #   mutate(days_in_crsp = tgt_rival_end_date - tgt_rival_start_date) %>% slice_max(order_by = days_in_crsp, n=1, with_ties = F) %>% ungroup() %>% select(-days_in_crsp)
  # 
  all_tgt_and_rivals_reg_id_info_aux3 <- all_tgt_and_rivals_reg_id_info_aux2 %>% group_by(merger_id, tgt_rival_gvkey, tgt_rival_permco) %>% slice_max(tgt_rival_start_date, n=1, with_ties = F) %>% ungroup()
  ...
  # of data points
  # all_tgt_and_rivals_reg_id_info_aux4 <- all_tgt_and_rivals_reg_id_info_aux3 %>% group_by(merger_id, tgt_rival_gvkey) %>% distinct(tgt_rival_permco, .keep_all = T) %>% 
  #   mutate(days_in_crsp = tgt_rival_end_date - tgt_rival_start_date) %>% slice_max(order_by = days_in_crsp, n=1, with_ties = F) %>% ungroup() %>% ungroup() %>% select(-days_in_crsp)
  all_tgt_and_rivals_reg_id_info_aux4 <- all_tgt_and_rivals_reg_id_info_aux3 %>% group_by(merger_id, tgt_rival_gvkey) %>% slice_max(tgt_rival_start_date, n=1, with_ties = F) %>% ungroup()
  ...
  all_tgt_and_rivals_reg_id_info_aux5 <- all_tgt_and_rivals_reg_id_info_aux4 %>% group_by(merger_id) %>% slice_max(order_by = tibble(year, score), 
                                                                                                                   n = num_of_rivals_per_firm, 
                                                                                                                   with_ties = F) %>%
    ungroup()
  
  # once more we need to correct for any merger ids we might have lost as some firms
  # might not have any rival with sufficient data points (see filtering for all_tgt_and_rivals_reg_id_info_aux2)
  # although those cases should be very rare
  missing_merger_ids <- setdiff(all_tgt_and_rivals_reg_id_info_aux1$merger_id, all_tgt_and_rivals_reg_id_info_aux5$merger_id)
  
  # for those merger ids we merely assign an NA to the columns imported from Hoberg-Philips dataset 
  missing_data <- all_tgt_and_rivals_reg_id_info_aux1 %>% 
    filter(merger_id %in% missing_merger_ids) %>% group_by(merger_id, tgt_permco) %>% slice(1) %>%
    mutate( year = NA,
            gvkey2 = NA,
            score = NA,
            tgt_rival_end_date = as.Date(NA), tgt_rival_start_date = as.Date(NA),
            tgt_rival_name = as.character(NA), tgt_rival_cusip = as.character(NA), 
            tgt_rival_ncusip = as.character(NA), tgt_rival_ticker = as.character(NA)
    )
  
  # finally add them back to the dataset
  all_tgt_and_rivals_reg_id_info_aux6 <- bind_rows(all_tgt_and_rivals_reg_id_info_aux5, missing_data)
  
  # finaly, rename some columns
  # this is important for we will then merge this dataframe with that of the targets
  # so it is important to distinguish which id info belongs to which firm (target or target) competitor
  all_tgt_and_rivals_reg_id_info <- all_tgt_and_rivals_reg_id_info_aux6  %>%
    rename(tgt_rival_score=score,
           tgt_year_hp = year)
  
  # and store the updated M&A dataset locally
  saveRDS(all_tgt_and_rivals_reg_id_info, "data/processed_data/id_data/tgt_and_rivals_id_data.rds")
...
tgt_and_rivals_id_info <- get_and_append_tgt_rivals_id_data(Sys.getenv("working_dir"),
